...
varespec.bray <- vegdist(TablaCompleta[,-c(1:4)], method = "euclidean") # dissimilarity matrix using bray-curtis distance indices on the varespec dataset native to vegan
pcoaVS <- pco(varespec.bray, negvals = "zero", dround = 0)
pcoaVS$vectors[,1:2]

f<-data.frame(pcoaVS$vectors[,1:2])
f <- cbind(TablaCompleta[,c(1:3,23)],f )
