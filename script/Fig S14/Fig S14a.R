...
for(treat in melted_Table$Nutrition  %>% unique){
  melted_sub <- melted_Table %>% subset(Nutrition  ==  treat) %>% droplevels
  ...
  for(var in melted_sub2$variable  %>% unique){
    ...
    NB = data.frame(t(melted_sub3[melted_sub3$Microbiome !='B+',][,7]))
    B = data.frame(t(melted_sub3[melted_sub3$Microbiome =='B+',][,7]))
    ...
    result3 = cbind(treat,result2)
    log2foldchange_Table <- rbind(log2foldchange_Table,result3)
  }}}
...
colnames(log2foldchange_Table_Complete)=c('Root_traits','Nutrition','Species_ID','diff','pvalue')
...
display <- log2foldchange_Table_Complete4  %>%
  acast(formula = Species_ID~Root_traits~Nutrition, mean,
        value.var = 'diff')
...
log2foldchange_Table_Complete4$Species_ID = factor(log2foldchange_Table_Complete4$Species_ID, c('LA7339','LP8539', 'LJ9250','LP0049', 'SI7820', 'SP9509'  ))
log2foldchange_Table_Complete4$Nutrition = factor(log2foldchange_Table_Complete4$Nutrition, rev(c('Medium','Low', 'High')))

log2foldchange_Table_Complete4$diff[which(log2foldchange_Table_Complete4$diff < -2.5)] <- "-2.5"
log2foldchange_Table_Complete4$diff[which(log2foldchange_Table_Complete4$diff > 2.5)] <- "2.5"
log2foldchange_Table_Complete4$diff = as.numeric(log2foldchange_Table_Complete4$diff)

head(log2foldchange_Table_Complete4)

ggplot(data = log2foldchange_Table_Complete4, aes(Root_traits, Species_ID)) + #plt_hmap = 
  geom_raster(aes(fill = diff)) +
  geom_tile(aes(color = Significance),fill = '#00000000', linewidth = 0.3,width = 0.9,height = 0.9) + #
  #geom_text(aes(label = Phylum),color = "black",size = 5)+
  facet_grid(Nutrition~., space = "free",scales = "free") +
  theme_few()+
  scale_fill_paletteer_c("pals::kovesi.diverging_bwr_40_95_c42", #kovesi.diverging_bwr_55_98_c37
                         limits = c(-2.5,2.5),na.value = "#D9D9D9",name = "Fold Change") +
  scale_color_manual(values = c('gray',"black"),na.value =  "transparent",name = "Significance vs NB") + #Significance Genotype vs Col-0
