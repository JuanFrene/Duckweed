...
seqs <- readDNAStringSet("Duckweed Metabolites.fas", format = "fasta")
...
metadata <- read.csv("Metadata Metabolites Exp.csv", sep=',')
...
dat <- data.frame(id=c(2,9,3,4), type=c('Landoltia','Lemna','Spirodela','Spirodela'))

f = ggtree(treeD, layout="circular", color='black', size=0.3) %<+% metadata + 
  geom_tiplab(aes(label=Species_ID), offset=0.01, hjust = 0.5, vjust=0.5, size=3.5, align = TRUE, linetype = "dotted", linesize = 0.15) +
