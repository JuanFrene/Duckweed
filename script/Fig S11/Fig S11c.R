...
Metabolome1_log2 = Metabolome1_log1[Metabolome1_log1$Species != "LP8539",]
Metabolome1_log2$Complexity = as.factor(Metabolome1_log2$Complexity)
...
##Save the file
#write.table(list_log2foldchange2, "list_log2foldchange Metabolites.txt")

list_log2foldchange2 = read.table("list_log2foldchange Metabolites.txt", header = TRUE, row.names = 1)

###Subsample
changes_metabolites_Pval <- list_log2foldchange2[list_log2foldchange2$SignPval ==  'q < 0.05',]
metabolites_list_Pval = data.frame(unique(changes_metabolites_Pval$Metabolite))
metabolites_list_Pvalmas1 = changes_metabolites_Pval[changes_metabolites_Pval $ log2foldchange > 0.1 ,]
metabolites_list_Pvalmenos1 = changes_metabolites_Pval[changes_metabolites_Pval $ log2foldchange < -0.1 ,]

metab_enhanced = rbind(metabolites_list_Pvalmas1,metabolites_list_Pvalmenos1)
nrow(metab_enhanced)
metab_enhanced_list = data.frame(unique(metab_enhanced$Metabolite))
colnames(metab_enhanced_list) = c('Metabolite')
unique(changes_metabolites_Pval$Metabolite)

changes_metabolites_Pajus <- list_log2foldchange2[list_log2foldchange2$SignPajus !=  'NoSignificant',]
metabolites_list_Pajus = unique(changes_metabolites_Pajus$Metabolite)
###12 metabolitos pasaron el filtro del FDR y ninguno esta en mas de una espcie.
###Hay un solo metabolito que aumenta mas en presencia de la syncom: Capryloylglycine es un antimicrobiano usado en cosmetologia



Enhanced_metabolites_ajust_only = merge(list_log2foldchange2,metab_enhanced_list, by = 'Metabolite')

###Clusterization
display <- Enhanced_metabolites_ajust_only  %>%
...
dend_nutr_data$labels
Enhanced_metabolites_ajust_only$Metabolite=factor(Enhanced_metabolites_ajust_only$Metabolite, nutr_order)
Enhanced_metabolites_ajust_only$Species=factor(Enhanced_metabolites_ajust_only$Species, c('SP7498','SI7820','LT9243','LA7339','SP9509','LY9205','SI9227','LJ9250','LP0049')) #species_order
nrow(list_log2foldchange2)

Enhanced_metabolites_ajust_only$log2foldchange[which(Enhanced_metabolites_ajust_only$log2foldchange > 0.05)] <- 0.05
Enhanced_metabolites_ajust_only$log2foldchange[which(Enhanced_metabolites_ajust_only$log2foldchange < -0.15)] <- -0.15

####Heatmap
plt_hmap = ggplot(data = Enhanced_metabolites_ajust_only, aes(Species,Metabolite)) +
