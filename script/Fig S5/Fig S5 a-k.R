...
setwd("G:/My Drive/labs/Nottingham/Duckweed/Figuras paper/Clean data/Fig S2/Fig S2a/")

Table = read.table("Root traits.txt", header = TRUE, row.names = 1)

Table$Microbiome = factor(Table$Microbiome, c('NB', 'B+'))

Table$Species = factor(Table$Species, c('PS','LY5280','SI9227','SP7498','SP9509',
...
Res_em <- NULL
Res_p <- NULL
for(specie in Table$Species %>% unique){
  melted_sub <- Table %>% subset(Species  == specie) %>% droplevels
...
S_C_NB_Mean = aggregate(CCL2__n_cell  ~Species,Table[Table$Microbiome=='NB',],mean)
S_C_B_Mean = aggregate(CCL2__n_cell  ~Species,Table[Table$Microbiome=='B+',],mean)
