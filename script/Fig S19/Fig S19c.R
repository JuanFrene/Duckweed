...
seqtab <- readRDS("G:/My Drive/labs/Nottingham/Duckweed/Figuras paper/Clean data/Fig 19/Fig 19c/ps.SR.rds")


####PCoA
ps.SR.Low = subset_samples(seqtab, Root ==  "Low")
ps.SR.Medium = subset_samples(seqtab, Root ==  "Medium")
ps.SR.High = subset_samples(seqtab, Root ==  "High")

PCoA <- ordinate(ps.SR.Low, "PCoA", "bray")
...
ggplot(data = PCoAmean4[PCoAmean4$Species!="Control",], aes(mean.x, mean.y, name=Species)) +theme_few()+
  ...
  geom_errorbarh(mapping = aes(xmin =mean.x - ds.x, xmax = mean.x + ds.x),size = 0.02,alpha = 0.4) +
  geom_errorbar(mapping = aes(ymin =mean.y - ds.y, ymax = mean.y + ds.y),size = 0.02,alpha = 0.4)  + 
