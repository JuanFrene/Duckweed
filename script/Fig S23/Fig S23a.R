...
Control = ggplot(melt.Species3[melt.Species3$Compound=='None',], aes(x=variable, y=value, fill=Species)) + 
  geom_point(aes(col=Species)) +
  theme_few()+ 
  theme(legend.position="right") +   # draw points
  ylim(NA,10)+
  geom_smooth(method="loess", se=T, level = 0.8, aes(col=Species)) + 
  labs(title='Control',  y='Area (cm )', x= 'Time (days)')+
  scale_color_manual(values = paleta_alive)

Lis = ggplot(melt.Species3[melt.Species3$Compound=='Lysine',], aes(x=variable, y=value, fill=Species)) + 
  geom_point(aes(col=Species)) +
  theme_few()+ 
  theme(legend.position="right") +
  ylim(NA,10)+# draw points
  geom_smooth(method="loess", se=T, level = 0.8, aes(col=Species)) + 
  labs(title='Lysine',  y='Area (cm )', x= 'Time (days)')+
  scale_color_manual(values = paleta_alive)

Citru = ggplot(melt.Species3[melt.Species3$Compound=='Citrulline',], aes(x=variable, y=value, fill=Species)) + 
  geom_point(aes(col=Species)) +
  theme_few()+ 
  theme(legend.position="right") +
  ylim(NA,10)+# draw points
  geom_smooth(method="loess", se=T, level = 0.8, aes(col=Species)) + 
  labs(title='Citru',  y='Area (cm )', x= 'Time (days)')+
  scale_color_manual(values = paleta_alive)

ggarrange(Control,Lis,Citru,common.legend = TRUE, legend = 'top', 
          ncol = 3, nrow=1)
