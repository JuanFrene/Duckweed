...
Metabolome_TN <- read.table("JGI targeted negative.txt", header = TRUE, row.names = 1)
Metabolome_TN2 = Metabolome_TN[Metabolome_TN$Exp!='Mono',]

melted_Metabolome_TN2 <- Metabolome_TN2 %>% melt
...
head(melted_Metabolome_TN2)
melted_Metabolome_TN2$value = as.numeric(melted_Metabolome_TN2$value)

log2foldchange <- c()
for(specie in melted_Metabolome_TN2$Species  %>% unique){
  melted_sub <- melted_Metabolome_TN2 %>% subset(Species ==  specie) %>% droplevels
