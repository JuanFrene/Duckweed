...
ps.3.Root.perc<- transform_sample_counts(ps.3.Root, function(x) x / sum(x))

ps.3.Root.2 = subset_samples(ps.3.Root.perc, ID !=  "R58")
...
PCoA <- ordinate(ps.3.Root.4, "PCoA", "bray")
...
x <- merge(x,ps.3.Root.4@sam_data, by=0)
