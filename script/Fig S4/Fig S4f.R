...
ps.3.Root.2 = subset_samples(ps.3.Root, ID !=  "R58")
ps.3.Root.3 = subset_samples(ps.3.Root.2, ID !=  "R65")
ps.3.Root.4 = subset_samples(ps.3.Root.3, ID !=  "R65_(2)")


#####PCoA for ASV-level data with Bray-Curtis
PCoA.F <- ordinate(ps.3.Root.4, "PCoA", "bray")
f<-data.frame(PCoA.F$vectors[, 1:2])
f <- merge(f,ps.3.Root.4@sam_data, by=0)
...
Root <- merge(PCoA,PCoAsd, by="Specie.1")

all = merge(Root_trait2[Root_trait2$Microbiome=='B+',],Root, by='Species' )


ggplot(data = all, aes(mean.x, Root )) +
...
xB2 = cbind(xB,Plant)
charla = merge(xB2,R, by ='Plant')
root.abund = vegdist(charla$PC1, method = "euclidean")
rootmicro.abund = vegdist(charla$Axis.1, method = "bray")
Root.root = mantel(root.abund, rootmicro.abund, method = "spearman", permutations = 9999, na.rm = TRUE)
#Mantel statistic r: 0.1181 Significance: 0.0162
